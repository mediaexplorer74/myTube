...
namespace myTube
{
  public static class MyMath
  {
    ...
    public static double Matchsigns(double signNumber, double outputNumber) 
            => signNumber > 0.0 && outputNumber < 0.0 || signNumber < 0.0 && outputNumber > 0.0 ? (outputNumber *= -1.0) : outputNumber;

    public static float RandomRange(float num1, float num2) 
            => (float) MyMath.rand.NextDouble() * (num2 - num1) + num1;
    ...
    public static float BetweenValue(float Val1, float Val2, float Between) 
            => (float) (((double) Between - (double) Val1) / ((double) Val2 - (double) Val1));
